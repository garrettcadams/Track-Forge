...
class recover {
	...
	function changePassword($username, $password, $salt) {
		...
		if ($result->num_rows > 0) {
			...
			// Change the password
			$stmt = $this->db->prepare("UPDATE `users` SET `password` = md5('{$password}'), `salted` = '' WHERE `username` = '{$this->db->real_escape_string(mb_strtolower($username))}'");
